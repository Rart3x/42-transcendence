...
export async function removeChannel(channelName : string, jwtToken : string) { 
	try { 
		const response = await fetch(`http://2F1.42angouleme.fr:3000/channel/delete/${channelName}`, { 
			method: "DELETE", 
			headers: { 
...
export async function unmuteUser(channelName : string, userName : string, jwtToken : string) {
	try {
		const response = await fetch(`http://2F1.42angouleme.fr:3000/channel/unmute/${channelName}/${userName}`, {
			method: "DELETE",
			headers: {
				"Content-Type": "application/json",
				"Authorization": `Bearer ${jwtToken}`
...
export async function removeFriend(userName: string, friendName: string, jwtToken : string) {
	try {
		const response = await fetch(`http://2F1.42angouleme.fr:3000/user/friend/delete/${friendName}`, {
			method: "DELETE",
			headers: {
				"Content-Type": "application/json",
				"Authorization": `Bearer ${jwtToken}`
...
export async function removeOperator(channelName : string, operatorName : string, jwtToken : string){
	try {
		const response = await fetch(`http://2F1.42angouleme.fr:3000/channel/${channelName}/operator/delete/${operatorName}`, {
			method: "DELETE",
			headers: {
				"Content-Type": "application/json",
				"Authorization": `Bearer ${jwtToken}`
...
export async function removeUserFromChannel(channelName: string, friendName: string, jwtToken : string) {
	try {
		const response = await fetch(`http://2F1.42angouleme.fr:3000/channel/${channelName}/delete/${friendName}`, {
			method: "DELETE",
			headers: {
				"Content-Type": "application/json",
				"Authorization": `Bearer ${jwtToken}`
...
export async function deleteGameRoomById(gameRoomId: string, jwtToken : string) {
	try {
	  const response = await fetch(`http://2F1.42angouleme.fr:3000/gameroom/deleteGameRoom/${gameRoomId}`, {
		method: "DELETE",
		headers: {
		  "Content-Type": "application/json",
		  "Authorization": `Bearer ${jwtToken}`
...
export async function deleteUser(userName: string, jwtToken : string) {
	try { 
		const response = await fetch(`http://2F1.42angouleme.fr:3000/user/delete/${userName}`, {
			method: "DELETE",
			headers: {
				"Content-Type": "application/json",
				"Authorization": `Bearer ${jwtToken}`
